...
public:
  ...
  DCTarget(Arc<RNic> nic, const QPConfig& config)
    : my_config(config)
  {
    // 1. create cq
...
public:
  ...
  static Option<Arc<DCTarget>> create(Arc<RNic> nic, const QPConfig& config)
  {
...
class DC
  : public Dummy
  , public std::enable_shared_from_this<DC>
{
public:
  const QPConfig my_config;
    ...
    , my_config(config)
  {
    ...
    if (!bring_dc_to_init(qp) || !bring_dc_to_rtr(qp) || !bring_dc_to_rts(qp)) {
      RDMA_ASSERT(false);
    }
  }
  ...
  struct ReqDesc
  {
    // `op` could be IBV_EXP_WR_RDMA_READ / IBV_EXP_WR_RDMA_WRITE /
    // IBV_EXP_WR_RDMA_ATOMIC
  ...
  Result<std::string> post_send(const ReqDesc& desc, const ReqPayload& payload)
  {
  ...
  Result<ibv_wc> wait_one_comp(
    const double& timeout = ::rdmaio::Timer::no_timeout())
  {
    Timer t;
  ...
  static bool bring_dc_to_init(ibv_qp* qp)
  {
    ...
    long flags = IBV_EXP_QP_STATE | IBV_EXP_QP_PKEY_INDEX | IBV_EXP_QP_PORT |
                 IBV_EXP_QP_DC_KEY;
  ...
  static bool bring_dc_to_rtr(ibv_qp* qp)
  {
    ...
    long flags = IBV_EXP_QP_STATE | IBV_EXP_QP_PATH_MTU | IBV_EXP_QP_AV;
  ...
  ~DC()
  {
    if (this->qp != NULL) {
      ...
      RDMA_VERIFY(WARNING, rc == 0)
        << "Failed to destroy DCT QP " << strerror(errno);
    }

    if (this->cq != NULL) {
      int rc = ibv_destroy_cq(this->cq);
      RDMA_VERIFY(WARNING, rc == 0)
        << "Failed to destroy DCT CQ " << strerror(errno);
    }
  }
};
